<template>
	<view class="HMfilterDropdown" :class="{'setDropdownBottom':maskVisibility}"  @touchmove.stop.prevent="discard" @tap.stop="discard">
		<view class="nav">
		...
		<view class="mask" :class="{'show':isShowMask}" @tap="togglePage(showPage)" v-if="isShowMask"></view>
		<block v-for="(page,page_index) in subData" :key="page_index">
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				<block v-if="page.type=='hierarchy'&& page.children.length>0">
					...
					<block v-for="(sub,index) in page.children" :key="index">
						<scroll-view class="sub-menu-list not-first" :scroll-y="true" v-if="activeMenuArr[page_index][0]==index&&sub.children.length>0"
...
<!-- 									<view class="more-sub-menu" v-if="sub_second.children&&sub.children.length>0&&sub_second.children.length>0">
										<block v-for="(sub2,sub2_index) in sub_second.children" :key="sub2_index">
											...
											<text v-if="sub_second.showAllSub!=true && sub2_index==8 && sub_second.children.length>9" @tap="showMoreSub(second_index)">更多<text
				...
				<block v-if="page.type=='filter'">
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.children" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
									<view v-for="(label,label_index) in box.children" :key="label_index" @tap="selectFilterLabel(page_index,box_index,label_index)"
					...
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.children" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
									<view v-for="(label,label_index) in box.children" :key="label_index" @tap="selectRadioLabel(page_index,box_index,label_index)"
									 :class="{'on':label.selected}">{{label.name}}</view>
								</view>
							</view>
						</scroll-view>
		...
		methods: {
			initMenu() {
				...
				for (let i = 0; i < this.filterData.length; i++) {
					...
					tmpMenu.push({
						...
						name: tmpitem.name || (tmpitem.type == "filter" ? "筛选" : tmpitem.children[0].name),
						type: tmpitem.type
					});
					//初始化选中项数组-ui状态
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				...
				if (level3_index != null || level2_index != null || (level1_index != null && this.subData[page_index].children[level1_index].children.length == 0)
					...
					if(this.updateMenuName){
						this.menu[page_index].name = (level3_index != null && sub.children[level3_index].name) || (level2_index != null && sub.name) || this.subData[page_index].children[level1_index].name;
					}
			...
			confirm() {
				...
				let id = JSON.parse(JSON.stringify(this.shadowActiveMenuArr));
					...
					if (typeof(item[0]) == 'object') {
						...
						item.forEach((s, j) => {
							if(s!=null){
								...
								item[j] = s;
								s.forEach((v, k) => {
									id[i][j][k] = (v==null||v>=this.subData[i].children[j].children.length)?null:this.subData[i].children[j].children[v].id;
									if(this.subData[i].type == 'radio' && id[i][j][k] == null){
										id[i][j] = [];
										index[i][j] = [];
									}
								});
						...
						id[i][0] = children.id;
						if(id[i].length>=2  && item[1]!=null){
							if(children.children.length>0){
								children = children.children[item[1]];
								id[i][1] = children.hasOwnProperty('id')?children.id:null;
							}else{
								id[i][1] = null
							}
							if(id[i].length>=3 && item[2]!=null){
								if(children.children.length>0){
									children = children.children[item[2]];
									id[i][2] = children.hasOwnProperty('id')?children.id:null;
								}else{
									id[i][2] = null;
								}
							}
			...
			processPage(index) {
				...
				if (this.menu[index].type == 'filter') {
					...
					for (let i = 0; i < level; i++) {
						let box = this.subData[index].children[i].children;
						for (let j = 0; j < box.length; j++) {
							if (this.shadowActiveMenuArr[index][i].indexOf(j) > -1) {
								this.subData[index].children[i].children[j].selected = true;
							} else {
								this.subData[index].children[i].children[j].selected = false;
							}
						}
					}
			...
			processActive(tmpitem) {
				let tmpArr = []
				if (tmpitem.type == 'hierarchy'&&tmpitem.hasOwnProperty('children')&&tmpitem.children.length>0) {
					...
					let level = tmpitem.children.length;
					while (level > 0) {
						tmpArr.push([]);
						level--;
					}
...
<style lang="scss">
	...
	.sub-menu-class {
		...
		&.show {
			transform: translate3d(0, calc(60upx + 1upx), 0);
			position: fixed;
		}
	}
	.sub-menu-list {
